...
@contextmanager
def log_exceptions(logger_instance: logging.Logger):
    """Context manager to log exceptions with full tracebacks.
    ...
    with log_exceptions(logger):
        ...
    ```
    Any exception raised inside the block will be logged via
    :func:`log_exception` and re-raised.
    """
    try:
        yield
    except Exception:
        log_exception(logger_instance)
        raise
